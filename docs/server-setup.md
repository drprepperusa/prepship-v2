...
Run PrepShip V2 on a remote server against the current V1 SQLite database so you can test parity safely before any cutover.

## Recommended Layout

Keep V1 and V2 side by side so V2 can reference the existing V1 SQLite DB and any sibling-repo fallbacks cleanly.

Example:

```text
/srv/
  prepship/          # existing V1 repo
...
- npm
- access to the existing V1 SQLite database file
- access to the same ShipStation-related `secrets.json` values V1 uses
...
- `DB_PROVIDER=sqlite`
  Required for parity testing against the live V1 database.
- `SQLITE_DB_PATH`
  Must point to the current V1 SQLite DB file you want V2 to validate against.
- `PREPSHIP_SECRETS_PATH`
  Should point to the secrets file V2 should load. Do not commit this file.
- `PREPSHIP_V1_ROOT`
  Points to the sibling V1 repo and is used for transitional fallback paths.
...
## What To Point At In V1

The critical reference is the current V1 SQLite DB path used by production or staging.
...
If V1 keeps its DB elsewhere, point V2 directly at that file. Do not copy the DB to a second location unless you intentionally want to test against a snapshot instead of the current live file.
