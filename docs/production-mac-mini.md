...
Mirror the current legacy production pattern:

- run V2 services on the Mac mini
...
This matches how the legacy stack is currently described in its production README: `launchd` on macOS, localhost services, and Cloudflare Tunnel for remote access.
...
- `react` on `127.0.0.1:4014` when you are validating the V3 frontend
...
That keeps the external shape similar to the legacy stack while reducing accidental direct API exposure.
...
1. run V2 on the Mac mini against the current legacy SQLite DB
...
- do not let the legacy stack and V2 both own the same long-running sync boundary without an explicit plan
