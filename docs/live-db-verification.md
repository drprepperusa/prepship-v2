...
The legacy stack is the source of truth for the refactor. These are not blockers; they are the specific assumptions inferred from legacy code that should be confirmed once the real SQLite file is available.

## Orders Module

- `orders.items` is expected to be JSON compatible with `json_each(...)` queries used by legacy SKU selection logic.
...
- `shipments.providerAccountId` is assumed to be nullable and backfilled over time, matching legacy sync behavior.
...
## Schema Assumptions Taken Directly From The Legacy Stack
...
- `clients.storeIds` JSON array used to map stores to clients in multiple legacy queries

## Verification Tasks

- Run V2 `GET /api/orders`, `GET /api/orders/:id`, and `GET /api/orders/ids` against the real DB and compare with the legacy stack for known cases.
- Check whether any production-only indexes or columns exist beyond those visible in legacy source.
