...
export function bootstrapApi(env = process.env, overrides: BootstrapApiOverrides = {}) {
  ...
  const initServices = new InitServices(dataStore.initRepository, initMetadataProvider, EXCLUDED_STORE_IDS);
  ...
  const listOrdersService = new ListOrdersService(dataStore.orderRepository);
  const orderDetailsService = new OrderDetailsService(dataStore.orderRepository);
